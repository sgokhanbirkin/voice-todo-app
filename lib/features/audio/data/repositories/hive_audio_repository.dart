...
class HiveAudioRepository implements IAudioRepository {
  ...
  Future<AppResult<AudioEntity>> saveAudioMetadata(AudioEntity audio) async {
    try {
      ...
      return AppResult.failure(
        DatabaseFailure('Failed to save audio metadata: $e'),
      );
    }
  }
  ...
  Future<AppResult<AudioEntity?>> getAudioById(String id) async {
    try {
      ...
      return AppResult.failure(
        DatabaseFailure('Failed to get audio by ID: $e'),
      );
    }
  }
  ...
  Future<AppResult<List<AudioEntity>>> getAudioByTaskId(String taskId) async {
    try {
      ...
      final taskAudio = allAudio
          .where((audio) => audio.taskId == taskId)
          .toList();

      _logger.info(
        'Retrieved ${taskAudio.length} audio files for task: $taskId',
      );
  ...
  Future<AppResult<AudioEntity>> updateAudioMetadata(AudioEntity audio) async {
    try {
      ...
      return AppResult.failure(
        DatabaseFailure('Failed to update audio metadata: $e'),
      );
    }
  }
  ...
  Future<AppResult<AudioEntity>> updateUploadProgress(
    ...
    try {
      ...
      await _audioBox.put(id, updatedAudio);
      _logger.info(
        'Audio upload progress updated: $id -> ${(progress * 100).toStringAsFixed(1)}%',
      );
      return AppResult.success(updatedAudio);
    } catch (e) {
  ...
  Future<AppResult<List<AudioEntity>>> getAudioBySyncStatus(
    ...
    try {
      ...
      final allAudio = _audioBox.values.cast<AudioEntity>().toList();
      final filteredAudio = allAudio
          .where((audio) => audio.syncStatus == syncStatus)
          .toList();

      _logger.info(
        'Retrieved ${filteredAudio.length} audio files with sync status: $syncStatus',
      );
  ...
  Future<AppResult<List<AudioEntity>>> searchAudio(String query) async {
    try {
      ...
      _logger.info(
        'Audio search for "$query" returned ${filteredAudio.length} results',
      );
      return AppResult.success(filteredAudio);
    } catch (e) {
  ...
  Future<AppResult<AudioStatistics>> getAudioStatistics() async {
    try {
      ...
      final totalSize = allAudio.fold<int>(
        0,
      ...
      _logger.info(
        'Generated audio statistics: $totalFiles total files, $totalSize bytes',
      );
  ...
  Future<AppResult<String>> exportAudioMetadata(String format) async {
    try {
      ...
      if (format.toLowerCase() == 'json') {
        ...
        _logger.info(
          'Exported ${allAudio.length} audio metadata records to JSON',
        );
